...
function showOnly(id) {
  SCREENS.forEach(s => { const el=$(s); if(el) el.style.display = (s===id) ? '' : 'none'; });
}
function avatarURL(user) {
  return user.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.displayName||'U')}&background=1a1a1a&color=fff&size=80&bold=true`;
}
...
auth.onAuthStateChanged(async user => {
  ...
  try {
    ...
    if (!hasUN) {
      $('upAvatar').src=$('upAvatar').src=avatarURL(user);
...
$('btnGoogle').addEventListener('click', async () => {
  ...
  try { await auth.signInWithPopup(provider); }
    ...
    const m={'auth/popup-closed-by-user':'Sign-in cancelled.','auth/popup-blocked':'Popup blocked — allow popups for this site.','auth/network-request-failed':'Network error. Check your connection.','auth/cancelled-popup-request':'Only one sign-in at a time.'};
...
async function saveNewUsername() {
  ...
  const ve=validateU(val); if(ve){$('unError').textContent=ve;return;}
  ...
  try {
    ...
    $('btnSaveUsername').disabled=false; $('btnSaveUsername').textContent='Save & Continue';
  }
}
...
async function saveSettings(){
  ...
  const ve=validateU(val); $('sError').textContent=''; $('sSuccess').textContent='';
  ...
  try {
    ...
    console.error('Settings:',err); $('sError').textContent='Could not save. Please try again.';
  } finally {
    $('btnSaveSettings').disabled=false; $('btnSaveSettings').textContent='Save Changes';
  }
}
